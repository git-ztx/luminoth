...
from luminoth.utils.vars import get_saver  # variable_summaries,
...
class SSD(snt.AbstractModule):
    ...
    def _build(self, image, gt_boxes=None, is_training=True):
        ...
        cls_prob_list = []
        for ind, endpoint in enumerate(self._endpoints):
            all_anchors_list.append(adjust_bboxes(
        ...
        if gt_boxes is not None:
            ...
            proposals_target, bbox_offsets_target = self._target(
        ...
        proposal_prediction = self._proposal(
            ...
            tf.cast(tf.shape(image)[1:3], tf.int32)
        )
    ...
    def loss(self, prediction_dict):
        ...
        with tf.name_scope('losses'):
            ...
            reg_loss_per_proposal = smooth_l1_loss(
                bbox_flatten, bbox_offsets_target_labeled)
            ...
            final_loss = (
                ...
                tf.cast(tf.shape(bbox_flatten)[0], tf.float32)
            )
    ...
    def generate_anchors_per_endpoint(self, anchors_per_point, endpoints,
                                      endpoints_outputs, min_scale,
                                      max_scale, ratios):
        """
